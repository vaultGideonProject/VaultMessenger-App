...
class MainActivity : ComponentActivity() {
    private val userRepository by lazy { SharedUserRepository(this) }
  ...
  // val database by lazy { AppDatabase.getDatabase(this) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        FirebaseService.initialize( useEmulators = launchConfigs.getEnv())
    ...
    override fun onRestart() {
        ...
        if (userId != null) {
            ...
            lifecycleScope.launch {
                // Update the user's status to "online" when the activity is restarted
...
@Composable
fun DrawerContent(user: LocalUser?, navController: NavHostController, context: Context) {
    ModalDrawerSheet {
        Column(
            modifier = Modifier
...
fun ProfileRow(navController: NavHostController) {
    Row(
        ...
        modifier = Modifier
            .padding(horizontal = 16.dp)
            .combinedClickable(
                onClick = { navController.navigate("profile/repeat")
                }
            )
    ) {
        Icon(
